...
class SyntaxCheckTestCase(unittest.TestCase):
    def test_correct_syntax_success(self):
        self.assertTrue(check_syntax(syntax_grammar_path, resources_path + 'script_correct_example.txt'))

    def test_incorrect_syntax_fail(self):
        self.assertFalse(check_syntax(syntax_grammar_path, resources_path + 'script_incorrect_example.txt'))
